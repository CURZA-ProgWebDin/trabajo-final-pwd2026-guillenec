<template>
  <main class="min-h-screen bg-linear-to-br from-[#09141D] via-[#10202B] to-[#183647] px-6 py-8">
    <section class="mx-auto max-w-7xl">
      <header class="mb-8">
        <p class="text-sm font-semibold uppercase tracking-[0.3em] text-[#86E7D3]">Admin</p>

        <h1 class="mt-2 text-4xl font-black text-[#EAF4F6]">Proveedores</h1>

        <p class="mt-2 max-w-3xl text-[#9DB4BE]">
          Vista base para gestionar proveedores. La lógica de listado, alta, edición y baja se
          implementará en el próximo paso.
        </p>
      </header>

      <div class="rounded-3xl border border-[#28414F] bg-[#13222D]/80 p-8 text-[#C9D7DC]">
        <p>CRUD de proveedores pendiente de implementar.</p>
      </div>
    </section>
  </main>
</template>
